# -*- coding: utf-8 -*-
"""
Created on Tue Jul 25 14:52:27 2023
...
path_default = 'F:\\cptac jia chun'
path_result_save = 'results gashissdb to cptac NOT normalized n BALANCED'

patch_size  = 224

keyword = 'cptac'
path[keyword] = 'F:\\cptac jia chun\\images'
TRAIN_DIR[keyword] = os.path.join(path[keyword], 'train augmented BALANCED')
VALID_DIR[keyword] = os.path.join(path[keyword], 'validation')
TEST_DIR[keyword] = os.path.join(path[keyword], 'test')

keyword_list = ['gashissdb', 'cptac']
...
#### transferring models
'''
path_result_save_origin = 'F:\\journal 3 experiment\\gashissdb as pretrained dataset\\journal 3 gashissdb 160 pretrained results'

patch_size_origin = 160
white_threshold = 0.1
cancerous_threshold = 0
additional_note = ''
additional_properties_origin = '_' + str(white_threshold) + '_' + str(cancerous_threshold) + '_' + additional_note

transfer_model_to_new_dataset(model_names, patch_size_origin, patch_size, additional_properties_origin, path_result_save_origin, 
...
std_preset = np.array([[[29.253716, 29.253716, 29.253716]]], dtype=np.float32)
mean_preset = np.array([[[3.34289, 3.34289, 3.34289]]], dtype=np.float32)
...
for keyword_no, keyword in enumerate(keyword_list):
    if continue_dataset <= keyword_no:
        for model_name in model_names:
            ...
            finetuned_model = tensorflow.keras.models.load_model(model_path)
            '''
            model_path = 'F:\\cptac jia chun\\results gashissdb to cptac NOT normalized\\resnet101 gashissdb+cptac\\13.h5'
            finetuned_model = tensorflow.keras.models.load_model(model_path)
            '''
            #for count in range (13, no_epoch_total):
...
table_complete = table_complete[:,2:]
