# -*- coding: utf-8 -*-
"""
Created on Tue Jul 25 14:16:43 2023
...
path_result_save = 'journal 2 experiment\\journal 2 extended experiment\\dataset chaoyang\\results gashissdb to chaoyang NOT normalized'

patch_size  = 256

keyword = 'chaoyang'
path[keyword] = "E:\\yongmingping\\GasHisSDB\\journal 2 experiment\\journal 2 extended experiment\\dataset chaoyang\\class_divided_include_validation"
TRAIN_DIR[keyword] = os.path.join(path[keyword], 'train augmented')
VALID_DIR[keyword] = os.path.join(path[keyword], 'validation')
TEST_DIR[keyword] = os.path.join(path[keyword], 'test')

keyword_list = ['gashissdb', 'chaoyang']
...
#### transferring models
'''
path_result_save_origin = 'E:\\yongmingping\\GasHisSDB\\journal 2 experiment\\journal 2 part 1\\gashissdb pretrained results no normalization'
...
transfer_model_to_new_dataset(model_names, patch_size_origin, additional_properties_origin, path_result_save_origin, 
...
for i, model_name in enumerate(model_names_w_details):
    model_path = os.path.join(path_default, path_result_save, model_name, str(int(table_original[i][0])) + '.h5')
    finetuned_model = tensorflow.keras.models.load_model(model_path)
